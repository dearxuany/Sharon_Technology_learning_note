...
# SQL函数
字段：</br>
在执行查询操作时，由函数新建出来的column并不真实存在于原始数据表中，用的时候才会自动生成，这个被生成的column被称为 “字段 field”。</br>
由计算得出的结果被称为“计算字段”，支持使用内建函数和一般的算术运算来生成。</br>
由字符串拼接所得的结果被称为“拼接字段”，支持内建函数或一些通配符来生成，而 MySQL 貌似仅能通过函数来生成。</br>
</br>
注意：</br>
