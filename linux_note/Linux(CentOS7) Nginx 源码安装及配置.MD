...
#error_log  logs/error.log;  # 打开错误日志
...
http {
    ...
    access_log  logs/access.log  main;  # 打开access.logs，格式为 main
 ...
 server {
        ...
        location / {
            ...
            index  index.html index.htm;  # 默认首页，优先级由签到后
        }
...
```
如果出现80端口被占用，可能是之前nginx已经被启动，ps aux|grep nginx 查进程，然后杀掉nginx再重启
